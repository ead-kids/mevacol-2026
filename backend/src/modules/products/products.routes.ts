...
productsRouter.get('/stats', requireRole(['ADMINISTRADOR']), (req: Request, res: Response): void => {
  try {
    const statsQuery = `
      SELECT 
...
productsRouter.get('/:id', (req: Request, res: Response): void => {
  try {
    ...
    const { id } = req.params;
    const product = db.prepare(`SELECT * FROM products WHERE id = ?`).get(String(id)) as any;
...
productsRouter.post('/', requireRole(['ADMINISTRADOR']), (req: Request, res: Response): void => {
  try {
    const {
      code,
      name,
    ...
    const existingCode = db.prepare(`SELECT id FROM products WHERE UPPER(code) = ?`).get(cleanCode);
    if (existingCode) {
      res.status(409).json({
        success: false,
...
productsRouter.put('/:id', requireRole(['ADMINISTRADOR']), (req: Request, res: Response): void => {
  try {
    const { id } = req.params;
    const {
      code,
    ...
    const codeConflict = db.prepare(`SELECT id FROM products WHERE UPPER(code) = ? AND id != ?`).get(cleanCode, String(id));
    if (codeConflict) {
      res.status(409).json({
        success: false,
...
productsRouter.patch('/:id/stock', requireRole(['ADMINISTRADOR']), (req: Request, res: Response): void => {
  try {
    const { id } = req.params;
...
productsRouter.patch('/:id/status', requireRole(['ADMINISTRADOR']), (req: Request, res: Response): void => {
  try {
    const { id } = req.params;
