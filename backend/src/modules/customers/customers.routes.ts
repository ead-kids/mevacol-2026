...
customersRouter.get('/', (req: Request, res: Response): void => {
  try {
...
customersRouter.post('/', requireRole(['ADMINISTRADOR', 'VENDEDOR']), (req: Request, res: Response): void => {
  try {
    ...
    const existing = db.prepare(`SELECT id FROM customers WHERE LOWER(id_number) = LOWER(?)`).get(cleanIdNumber);
    if (existing) {
      res.status(400).json({
        success: false,
...
customersRouter.put('/:id', requireRole(['ADMINISTRADOR', 'VENDEDOR']), (req: Request, res: Response): void => {
  try {
    const { id } = req.params;
    const { name, id_number, phone, email, address, city, notes } = req.body;

    const existingCustomer = db.prepare(`SELECT id, id_number, name FROM customers WHERE id = ?`).get(String(id)) as any;
    if (!existingCustomer) {
      res.status(404).json({ success: false, error: 'Cliente no encontrado.' });
      return;
    }
...
customersRouter.patch('/:id/status', requireRole(['ADMINISTRADOR']), (req: Request, res: Response): void => {
  try {
    const { id } = req.params;
    const { is_active } = req.body;

    const customer = db.prepare(`SELECT id, name, is_active FROM customers WHERE id = ?`).get(String(id)) as any;
    if (!customer) {
      res.status(404).json({ success: false, error: 'Cliente no encontrado.' });
      return;
    }
