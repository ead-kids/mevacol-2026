...
// 3. Crear Cliente
customersRouter.post('/', requireRole(['ADMINISTRADOR', 'VENDEDOR']), async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const existing = await queryOne(`SELECT id FROM customers WHERE LOWER(id_number) = LOWER(?)`, [cleanIdNumber]);
    if (existing) {
    ...
    await queryRun(
      `INSERT INTO customers (id, id_number, name, phone, email, address, city, is_active, notes, created_by_user_id, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, NOW(), NOW())`,
      [customerId, cleanIdNumber, cleanName,
    ...
    recordAuditLog({ userId: req.user!.id, action: 'CUSTOMER_CREATED', entityName: 'customers', entityId: customerId, details: { name: cleanName, id_number: cleanIdNumber }, ipAddress: req.ip, deviceInfo: req.headers['user-agent'] });
...
// 4. Editar Cliente
customersRouter.put('/:id', requireRole(['ADMINISTRADOR', 'VENDEDOR']), async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { name, id_number, phone, email, address, city, notes } = req.body;

    const existingCustomer = await queryOne<any>(`SELECT id, id_number, name FROM customers WHERE id = ?`, [id]);
    if (!existingCustomer) { res.status(404).json({ success: false, error: 'Cliente no encontrado.' }); return; }

    if (!name || !id_number) { res.status(400).json({ success: false, error: 'El nombre y el número de documento son obligatorios.' }); return; }

    const cleanName = String(name).trim();
    const cleanIdNumber = String(id_number).trim();

    const duplicateDoc = await queryOne(`SELECT id FROM customers WHERE LOWER(id_number) = LOWER(?) AND id != ?`, [cleanIdNumber, id]);
    if (duplicateDoc) { res.status(400).json({ success: false, error: `Ya existe otro cliente con el documento/NIT: ${cleanIdNumber}.` }); return; }

    await queryRun(
      `UPDATE customers SET name = ?, id_number = ?, phone = ?, email = ?, address = ?, city = ?, notes = ?, updated_at = NOW() WHERE id = ?`,
      [cleanName, cleanIdNumber, phone ? String(phone).trim() : null, email ? String(email).trim().toLowerCase() : null, address ? String(address).trim() : null, city ? String(city).trim() : null, notes ? String(notes).trim() : null, id]
    );

    recordAuditLog({ userId: req.user!.id, action: 'CUSTOMER_UPDATED', entityName: 'customers', entityId: String(id), details: { name: cleanName, id_number: cleanIdNumber }, ipAddress: req.ip, deviceInfo: req.headers['user-agent'] });
...
customersRouter.patch('/:id/status', requireRole(['ADMINISTRADOR']), async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { is_active } = req.body;

    const customer = await queryOne<any>(`SELECT id, name, is_active FROM customers WHERE id = ?`, [id]);
    if (!customer) { res.status(404).json({ success: false, error: 'Cliente no encontrado.' }); return; }

    const newStatus = is_active ? 1 : 0;
    await queryRun(`UPDATE customers SET is_active = ?, updated_at = NOW() WHERE id = ?`, [newStatus, id]);

    recordAuditLog({ userId: req.user!.id, action: 'CUSTOMER_STATUS_UPDATED', entityName: 'customers', entityId: String(id), details: { customerName: customer.name, newStatus }, ipAddress: req.ip, deviceInfo: req.headers['user-agent'] });
