...
invoicesRouter.get('/stats', (req: Request, res: Response): void => {
  try {
...
invoicesRouter.get('/', (req: Request, res: Response): void => {
  try {
    const userRole = req.user!.role_code;
    const userId = req.user!.id;
...
invoicesRouter.get('/by-sale/:saleId', (req: Request, res: Response): void => {
  try {
    const userRole = req.user!.role_code;
    const userId = req.user!.id;
...
invoicesRouter.get('/:id', (req: Request, res: Response): void => {
  try {
    const userRole = req.user!.role_code;
    const userId = req.user!.id;
    const { id } = req.params;
...
invoicesRouter.post('/from-sale/:saleId', (req: Request, res: Response): void => {
  try {
    const userRole = req.user!.role_code;
    const userId = req.user!.id;
    const { saleId } = req.params;
    ...
    const existing = db.prepare(`SELECT * FROM invoices WHERE sale_id = ?`).get(saleId) as any;
    if (existing) {
      res.json({
        success: true,
