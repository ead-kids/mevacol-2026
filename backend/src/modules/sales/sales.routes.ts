...
salesRouter.get('/stats', (req: Request, res: Response): void => {
  try {
...
salesRouter.get('/', (req: Request, res: Response): void => {
  try {
    const userRole = req.user!.role_code;
    const userId = req.user!.id;
...
salesRouter.get('/:id', (req: Request, res: Response): void => {
  try {
    const userRole = req.user!.role_code;
    const userId = req.user!.id;
    const { id } = req.params;
    ...
    const sale = db.prepare(saleQuery).get(String(id)) as any;
    ...
    const items = db.prepare(itemsQuery).all(String(id));
...
salesRouter.post('/', (req: Request, res: Response): void => {
  try {
    const userRole = req.user!.role_code;
    const userId = req.user!.id;
    ...
    if (userRole === 'ADMINISTRADOR' && seller_id) {
      const sellerCheck = db.prepare(`SELECT id, full_name FROM users WHERE id = ?`).get(String(seller_id)) as any;
      if (sellerCheck) {
    ...
    const createSaleTransaction = db.transaction(() => {
      ...
      const customer = db.prepare(`SELECT id, name, is_active FROM customers WHERE id = ?`).get(String(customer_id)) as any;
      if (!customer) {
      ...
      const allSalesWithVta = db.prepare(`
      ...
      for (const item of items) {
        ...
        if (product.current_stock < qty) {
          throw new Error(
            `Stock insuficiente para '${product.name}' (${product.code}). Cantidad solicitada: ${qty}, Stock disponible: ${product.current_stock}.`
          );
        }
      ...
      const allInvoices = db.prepare(`SELECT invoice_code FROM invoices`).all() as Array<{ invoice_code: string }>;
      let maxFacNumber = 0;
