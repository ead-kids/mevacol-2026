...
usersRouter.get('/roles', async (req: Request, res: Response) => {
  try {
    const roles = await queryAll('SELECT code, name, description FROM roles');
...
usersRouter.post('/', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const roleExists = await queryOne('SELECT code FROM roles WHERE code = ?', [role_code]);
    if (!roleExists) {
      res.status(400).json({
        success: false,
...
usersRouter.patch('/:id/status', async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params;
    const { is_active } = req.body;

    const user = await queryOne<any>('SELECT id, username, role_code, is_active FROM users WHERE id = ?', [id]);
    if (!user) {
    ...
    if (user.role_code === 'ADMINISTRADOR' && newStatus === 0) {
      const activeAdmins = await queryOne<{ count: string }>(
        `SELECT COUNT(*) as count FROM users WHERE role_code = 'ADMINISTRADOR' AND is_active = 1`
      );
...
usersRouter.delete('/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const user = await queryOne<any>('SELECT id, username, role_code FROM users WHERE id = ?', [id]);
    if (!user) {
      res.status(404).json({ success: false, error: 'Usuario no encontrado.' });
      return;
    }
