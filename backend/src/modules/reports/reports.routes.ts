...
// ─── Reporte de Ventas ───────────────────────────────────────────────────────
reportsRouter.get('/sales', (req: Request, res: Response) => {
  try {
    ...
    const summary = {
      ...
      total_revenue_cop: rows.reduce((a: number, r: any) => a + (r.total_cop || 0), 0),
      total_discount_cop: rows.reduce((a: number, r: any) => a + (r.discount_cop || 0), 0),
    };
...
reportsRouter.get('/products-sold', (req: Request, res: Response) => {
  try {
    ...
    const rows = db.prepare(`
      SELECT
        ...
        COALESCE(AVG(si.unit_price_cop), 0) as avg_price_cop
      FROM sale_items si
    ...
    const summary = {
      total_rows: rows.length,
      total_units: rows.reduce((a: number, r: any) => a + (r.total_quantity || 0), 0),
      total_revenue_cop: rows.reduce((a: number, r: any) => a + (r.total_revenue_cop || 0), 0),
    };

    return res.json({ success: true, report: 'products-sold', summary, data: rows });
  } catch (err: any) {
    console.error('Error en reporte de productos vendidos:', err);
  ...
  try {
    ...
    const rows = db.prepare(`
      SELECT
        ...
        price_cop,
        cost_cop,
        (current_stock * cost_cop) as stock_cost_value,
        (current_stock * price_cop) as stock_retail_value,
        CASE
        ...
        updated_at
      FROM products
      WHERE 1=1 ${categoryFilter} ${stockFilter}
      ORDER BY category, name
    `).all() as any[];

    const summary = {
      total_rows: rows.length,
      total_units: rows.reduce((a: number, r: any) => a + (r.current_stock || 0), 0),
      total_cost_value: rows.reduce((a: number, r: any) => a + (r.stock_cost_value || 0), 0),
      total_retail_value: rows.reduce((a: number, r: any) => a + (r.stock_retail_value || 0), 0),
      low_stock_count: rows.filter((r: any) => r.stock_status === 'STOCK_BAJO').length,
      out_of_stock_count: rows.filter((r: any) => r.stock_status === 'AGOTADO').length,
    };
...
// ─── Reporte de Clientes ─────────────────────────────────────────────────────
reportsRouter.get('/customers', (req: Request, res: Response) => {
  try {
    ...
    const summary = {
      total_rows: rows.length,
      active_count: rows.filter((r: any) => r.is_active === 1).length,
      total_revenue_cop: rows.reduce((a: number, r: any) => a + (r.total_spent_cop || 0), 0),
    };
...
// ─── Reporte de Facturas ─────────────────────────────────────────────────────
reportsRouter.get('/invoices', (req: Request, res: Response) => {
  try {
    ...
    const summary = {
      ...
      total_revenue_cop: rows.reduce((a: number, r: any) => a + (r.total_cop || 0), 0),
      total_discount_cop: rows.reduce((a: number, r: any) => a + (r.discount_cop || 0), 0),
      avg_ticket_cop: rows.length ? Math.round(rows.reduce((a: number, r: any) => a + (r.total_cop || 0), 0) / rows.length) : 0,
    };

    return res.json({ success: true, report: 'invoices', summary, data: rows });
  } catch (err: any) {
    console.error('Error en reporte de facturas:', err);
    return res.status(500).json({ success: false, error: 'Error al generar reporte de facturas.' });
  }
});

// ─── Reporte de Entregas ──────────────────────────────────────────────────────
reportsRouter.get('/deliveries', (req: Request, res: Response) => {
  try {
    ...
    const statusLabels: Record<string, string> = {
      ...
      ENTREGADA: 'Entregada', NO_ENTREGADA: 'No Entregada', CANCELADA: 'Cancelada',
    };

    const summary = {
      total_rows: rows.length,
      delivered_count: rows.filter((r: any) => r.status === 'ENTREGADA').length,
      failed_count: rows.filter((r: any) => r.status === 'NO_ENTREGADA').length,
      pending_count: rows.filter((r: any) => r.status === 'PENDIENTE').length,
    };

    return res.json({ success: true, report: 'deliveries', summary, data: rows, statusLabels });
  } catch (err: any) {
    console.error('Error en reporte de entregas:', err);
    return res.status(500).json({ success: false, error: 'Error al generar reporte de entregas.' });
  }
});

// ─── Listados de filtros (auxiliares) ────────────────────────────────────────
reportsRouter.get('/filter-options', (req: Request, res: Response) => {
  try {
    const sellers = db.prepare(`SELECT id, full_name FROM users WHERE role_code = 'VENDEDOR' AND is_active = 1 ORDER BY full_name`).all();
    const deliverers = db.prepare(`SELECT id, full_name FROM users WHERE role_code = 'ENTREGADOR' AND is_active = 1 ORDER BY full_name`).all();
    const customers = db.prepare(`SELECT id, name, id_number FROM customers WHERE is_active = 1 ORDER BY name`).all();
    const products = db.prepare(`SELECT id, code, name, category FROM products WHERE is_active = 1 ORDER BY name`).all();
    ...
    console.error('Error en filter-options:', err);
