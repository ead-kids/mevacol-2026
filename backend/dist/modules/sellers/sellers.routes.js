...
exports.sellersRouter.use(auth_middleware_1.authMiddleware);
...
exports.sellersRouter.post('/location', async (req, res) => {
    try {
        ...
        try {
            await (0, db_1.queryRun)('ALTER TABLE seller_locations ADD COLUMN IF NOT EXISTS seller_user_id TEXT');
            await (0, db_1.queryRun)('ALTER TABLE seller_locations ADD COLUMN IF NOT EXISTS user_id TEXT');
            await (0, db_1.queryRun)('ALTER TABLE seller_locations ADD COLUMN IF NOT EXISTS is_active INTEGER DEFAULT 1');
            await (0, db_1.queryRun)('ALTER TABLE seller_locations ADD COLUMN IF NOT EXISTS updated_at TEXT');
        }
        catch { }
        // 1. Comprobar si ya existe registro de ubicación para este usuario
        const existing = await (0, db_1.queryOne)('SELECT id FROM seller_locations WHERE seller_user_id = ? OR user_id = ? LIMIT 1', [sellerId, sellerId]);
        if (existing) {
            await (0, db_1.queryRun)(`UPDATE seller_locations
           SET latitude = ?, longitude = ?, accuracy = ?, is_active = 1, updated_at = TO_CHAR(NOW(), 'YYYY-MM-DD HH24:MI:SS')
           WHERE id = ?`, [latitude, longitude, acc, existing.id]);
        }
        else {
            await (0, db_1.queryRun)(`INSERT INTO seller_locations (id, seller_user_id, user_id, latitude, longitude, accuracy, is_active, updated_at)
           VALUES (?, ?, ?, ?, ?, ?, 1, TO_CHAR(NOW(), 'YYYY-MM-DD HH24:MI:SS'))`, [(0, uuid_1.v4)(), sellerId, sellerId, latitude, longitude, acc]);
        }
        res.json({
            success: true,
...
exports.sellersRouter.post('/location/stop', async (req, res) => {
    try {
        const sellerId = req.user.id;
        try {
            await (0, db_1.queryRun)(`UPDATE seller_locations
           SET is_active = 0, updated_at = TO_CHAR(NOW(), 'YYYY-MM-DD HH24:MI:SS')
           WHERE seller_user_id = ? OR user_id = ?`, [sellerId, sellerId]);
        }
        catch (dbErr) {
...
exports.sellersRouter.get('/locations', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
...
exports.sellersRouter.get('/stats', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
    ...
    `);
        const salesStats = await (0, db_1.queryOne)(`
      SELECT
...
exports.sellersRouter.get('/', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
    ...
    `;
        ...
        const rawSellers = await (0, db_1.queryAll)(query, params);
...
exports.sellersRouter.get('/:id', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        const seller = await (0, db_1.queryOne)(`
      SELECT
    ...
    `, [id]);
        ...
        const metrics = await (0, db_1.queryOne)(`
      SELECT
        COUNT(*) as total_sales_count,
...
exports.sellersRouter.get('/:id/sales', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        const search = req.query.search ? String(req.query.search).trim().toLowerCase() : '';
        const status = req.query.status ? String(req.query.status).trim() : '';
        const sellerExists = await (0, db_1.queryOne)(`SELECT id, full_name FROM users WHERE id = ? AND role_code = 'VENDEDOR'`, [id]);
        if (!sellerExists) {
            res.status(404).json({ success: false, error: 'Vendedor no encontrado.' });
            return;
        }
...
exports.sellersRouter.post('/', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { username, full_name, document_id, password, email, phone, address, } = req.body;
...
exports.sellersRouter.put('/:id', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        const { full_name, document_id, phone, email, address, password } = req.body;
        const existing = await (0, db_1.queryOne)(`SELECT id, username FROM users WHERE id = ? AND role_code = 'VENDEDOR'`, [id]);
        if (!existing) {
            res.status(404).json({ success: false, error: 'Vendedor no encontrado.' });
            return;
        }
        ...
        if (password && String(password).trim().length >= 6) {
            const newHash = await bcryptjs_1.default.hash(String(password).trim(), 10);
...
exports.sellersRouter.patch('/:id/status', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        const { is_active } = req.body;
        const existing = await (0, db_1.queryOne)(`SELECT id, username, is_active FROM users WHERE id = ? AND role_code = 'VENDEDOR'`, [id]);
        if (!existing) {
            res.status(404).json({ success: false, error: 'Vendedor no encontrado.' });
            return;
        }
...
exports.sellersRouter.delete('/:id', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        const admin = await (0, db_1.queryOne)(`SELECT password_hash FROM users WHERE id = ?`, [req.user.id]);
        if (!admin || !(await bcryptjs_1.default.compare(admin_password, admin.password_hash))) {
            res.status(403).json({ success: false, error: 'Contraseña de administrador incorrecta.' });
            return;
        }
        const seller = await (0, db_1.queryOne)(`SELECT id, username FROM users WHERE id = ? AND role_code = 'VENDEDOR'`, [id]);
        if (!seller) {
            res.status(404).json({ success: false, error: 'Vendedor no encontrado.' });
            return;
        }
        // Verificar si tiene ventas registradas en el sistema
        const salesCount = await (0, db_1.queryOne)(`SELECT COUNT(*) as count FROM sales WHERE seller_user_id = ?`, [id]);
        ...
        await (0, db_1.queryRun)(`DELETE FROM users WHERE id = ? AND role_code = 'VENDEDOR'`, [id]);
        (0, db_1.recordAuditLog)({
            userId: req.user.id,
