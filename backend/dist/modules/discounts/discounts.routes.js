...
exports.discountsRouter = void 0;
const express_1 = require("express");
const uuid_1 = require("uuid");
const db_1 = require("../../database/db");
const auth_middleware_1 = require("../../middlewares/auth.middleware");
const role_middleware_1 = require("../../middlewares/role.middleware");
exports.discountsRouter = (0, express_1.Router)();
// Middleware general de autenticación para todas las rutas de descuentos
exports.discountsRouter.use(auth_middleware_1.authMiddleware);
 ...
 */
exports.discountsRouter.get('/active', async (req, res) => {
    try {
...
exports.discountsRouter.use((0, role_middleware_1.requireRole)(['ADMINISTRADOR']));
/**
 * GET /stats
 * Métricas globales del sistema de promociones y descuentos.
 */
exports.discountsRouter.get('/stats', async (req, res) => {
    try {
        const today = new Date().toISOString().substring(0, 10);
        const counts = await (0, db_1.queryOne)(`
      SELECT
    ...
    `);
        ...
        const savingsRow = await (0, db_1.queryOne)(`
      SELECT
 ...
 */
exports.discountsRouter.get('/', async (req, res) => {
    try {
 ...
 */
exports.discountsRouter.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
 ...
 */
exports.discountsRouter.post('/', async (req, res) => {
    try {
        const { code, name, description, discount_type, value, product_id, min_quantity, start_date, end_date, is_active, } = req.body;
        ...
        if (product_id && String(product_id).trim()) {
            ...
            const product = await (0, db_1.queryOne)(`SELECT id, name, price_cop FROM products WHERE id = ?`, [cleanProductId]);
            if (!product) {
        ...
        const existing = await (0, db_1.queryOne)(`SELECT id FROM discounts WHERE UPPER(code) = ?`, [cleanCode]);
        if (existing) {
 ...
 */
exports.discountsRouter.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, discount_type, value, product_id, min_quantity, start_date, end_date, is_active, } = req.body;
        const existing = await (0, db_1.queryOne)(`SELECT id, code FROM discounts WHERE id = ?`, [id]);
        if (!existing) {
            res.status(404).json({ success: false, error: 'Descuento no encontrado.' });
            return;
        }
        ...
        if (product_id && String(product_id).trim()) {
            cleanProductId = String(product_id).trim();
            const product = await (0, db_1.queryOne)(`SELECT id, price_cop FROM products WHERE id = ?`, [cleanProductId]);
            if (!product) {
 ...
 */
exports.discountsRouter.patch('/:id/status', async (req, res) => {
    try {
        const { id } = req.params;
        const { is_active } = req.body;
        const existing = await (0, db_1.queryOne)(`SELECT id, name, code, is_active FROM discounts WHERE id = ?`, [id]);
        if (!existing) {
            res.status(404).json({ success: false, error: 'Descuento no encontrado.' });
            return;
        }
        const newStatus = is_active === true || is_active === 1 || is_active === '1' ? 1 : 0;
        await (0, db_1.queryRun)(`UPDATE discounts SET is_active = ?, updated_at = NOW() WHERE id = ?`, [newStatus, id]);
        (0, db_1.recordAuditLog)({
            userId: req.user.id,
 ...
 */
exports.discountsRouter.delete('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const existing = await (0, db_1.queryOne)(`SELECT id, name, code FROM discounts WHERE id = ?`, [id]);
        if (!existing) {
            res.status(404).json({ success: false, error: 'Descuento no encontrado.' });
            return;
        }
        await (0, db_1.queryRun)(`DELETE FROM discounts WHERE id = ?`, [id]);
        (0, db_1.recordAuditLog)({
            userId: req.user.id,
