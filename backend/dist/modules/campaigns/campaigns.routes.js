...
exports.campaignsRouter.use(auth_middleware_1.authMiddleware);
/**
    ...
    try {
      ...
      `, [userId]);
            ...
            const enrichedCampaigns = await Promise.all(campaigns.map(async (c) => {
                ...
                const salesAgg = await (0, db_1.queryOne)(`
            SELECT
 ...
 */
exports.campaignsRouter.get('/', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
    ...
    `;
        ...
        const campaigns = await (0, db_1.queryAll)(query, params);
        ...
        const enriched = await Promise.all(campaigns.map(async (camp) => {
            ...
            const sellersList = await (0, db_1.queryAll)(sellersQuery, sellerParams);
            let winnersCount = 0;
            for (const s of sellersList) {
                const sellerSum = await (0, db_1.queryOne)(`
 ...
 */
exports.campaignsRouter.get('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const userRole = req.user.role_code;
        const userId = req.user.id;
        const campaign = await (0, db_1.queryOne)(`
    ...
    `, [id]);
      ...
      `;
        ...
        const assignedSellers = await (0, db_1.queryAll)(assignedSellersQuery, assignedParams);
        ...
        const sellersProgress = await Promise.all(assignedSellers.map(async (seller) => {
            const salesAgg = await (0, db_1.queryOne)(`
          SELECT
            COALESCE(SUM(total_cop), 0) as total_cop,
            COUNT(id) as count
 ...
 */
exports.campaignsRouter.post('/', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { name, description, target_amount_cop, reward_description, start_date, end_date, is_general, seller_ids, } = req.body;
 ...
 */
exports.campaignsRouter.put('/:id', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        const { name, description, target_amount_cop, reward_description, start_date, end_date, is_general, seller_ids, } = req.body;
        const existing = await (0, db_1.queryOne)('SELECT id FROM campaigns WHERE id = ?', [id]);
        if (!existing) {
            res.status(404).json({ success: false, error: 'Campaña no encontrada.' });
            return;
        }
      ...
      `, [
            ...
            await (0, db_1.queryRun)('DELETE FROM campaign_sellers WHERE campaign_id = ?', [id]);
            if (generalFlag === 0 && Array.isArray(seller_ids) && seller_ids.length > 0) {
                for (const sellerId of seller_ids) {
                    await (0, db_1.queryRun)(`
            INSERT INTO campaign_sellers (campaign_id, seller_user_id, created_at)
 ...
 */
exports.campaignsRouter.patch('/:id/status', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        const { is_active } = req.body;
        const campaign = await (0, db_1.queryOne)('SELECT id, name, is_active FROM campaigns WHERE id = ?', [id]);
        if (!campaign) {
            res.status(404).json({ success: false, error: 'Campaña no encontrada.' });
            return;
        }
        const newStatus = (is_active === true || is_active === 1 || is_active === '1') ? 1 : 0;
        await (0, db_1.queryRun)('UPDATE campaigns SET is_active = ? WHERE id = ?', [newStatus, id]);
        (0, db_1.recordAuditLog)({
            userId: req.user.id,
 ...
 */
exports.campaignsRouter.delete('/:id', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        const campaign = await (0, db_1.queryOne)('SELECT id, name FROM campaigns WHERE id = ?', [id]);
        if (!campaign) {
            res.status(404).json({ success: false, error: 'Campaña no encontrada.' });
            return;
        }
        await (0, db_1.withTransaction)(async () => {
            await (0, db_1.queryRun)('DELETE FROM campaign_sellers WHERE campaign_id = ?', [id]);
            await (0, db_1.queryRun)('DELETE FROM campaigns WHERE id = ?', [id]);
        });
        (0, db_1.recordAuditLog)({
            userId: req.user.id,
