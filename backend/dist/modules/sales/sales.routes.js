    ...
    try {
        ...
        `);
            ...
            for (const item of items) {
                ...
                const applicableDiscounts = activeDiscounts.filter((d) => (d.product_id === product.id || !d.product_id) && qty >= Number(d.min_quantity || 1));
                let unitDiscount = 0;
         ...
         VALUES (?, ?, ?, ?, 'REGISTRADA', ?, ?, ?, ?, 1, NOW(), NOW())`, [saleId, invoiceNumber, customer.id, sellerIdToRecord, calculatedSubtotal, calculatedDiscount, calculatedFinalTotal, notes ? String(notes).trim() : null]);
            // 5. Insertar items y descontar stock
            for (const vi of verifiedItems) {
                await txRun(`INSERT INTO sale_items (id, sale_id, product_id, quantity, unit_price_cop, discount_cop, total_cop, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`, [(0, uuid_1.v4)(), saleId, vi.product_id, vi.quantity, vi.unit_price_cop, vi.discount_cop, vi.total_cop]);
         ...
         VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'INTERNA', NOW(), NOW())`, [invoiceId, invoiceCode, saleId, customer.id, sellerIdToRecord, calculatedSubtotal, calculatedDiscount, calculatedFinalTotal, notes ? String(notes).trim() : null]);
            return {
                saleId,
                invoiceNumber,
                invoiceId,
