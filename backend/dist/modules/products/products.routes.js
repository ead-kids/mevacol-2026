...
exports.productsRouter.use(auth_middleware_1.authMiddleware);
// 1. Estadísticas y Métricas de Inventario (Solo ADMINISTRADOR)
exports.productsRouter.get('/stats', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
...
exports.productsRouter.get('/:id', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const product = await (0, db_1.queryOne)(`SELECT * FROM products WHERE id = ?`, [id]);
        if (!product) {
            res.status(404).json({ success: false, error: 'Producto no encontrado.' });
            return;
        }
        if (userRole === 'VENDEDOR')
            delete product.cost_cop;
...
exports.productsRouter.post('/', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        ...
        if (!code || !String(code).trim()) {
            res.status(400).json({ success: false, error: 'El código o referencia es obligatorio.' });
            return;
        }
        if (!name || !String(name).trim()) {
            res.status(400).json({ success: false, error: 'El nombre del producto es obligatorio.' });
            return;
        }
        if (price_cop === undefined || Number(price_cop) < 0) {
            res.status(400).json({ success: false, error: 'El precio de venta debe ser un número mayor o igual a 0.' });
            return;
        }
        const cleanCode = String(code).trim().toUpperCase();
        const existingCode = await (0, db_1.queryOne)(`SELECT id FROM products WHERE UPPER(code) = ?`, [cleanCode]);
        if (existingCode) {
            res.status(409).json({ success: false, error: `Ya existe un producto con el código '${cleanCode}'.` });
            return;
        }
        ...
        await (0, db_1.queryRun)(`INSERT INTO products (id, code, name, description, category, unit_measure, price_cop, cost_cop, current_stock, min_stock, is_active, image_url, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, ?, NOW(), NOW())`, [newId, cleanCode, cleanName, description ? String(description).trim() : null, String(category).trim() || 'General', String(unit_measure).trim() || 'Unidad', numPrice, numCost, numStock, numMinStock, cleanImageUrl]);
        (0, db_1.recordAuditLog)({ userId: req.user.id, action: 'PRODUCT_CREATE', entityName: 'products', entityId: newId, details: { code: cleanCode, name: cleanName, price_cop: numPrice, cost_cop: numCost, current_stock: numStock }, ipAddress: req.ip, deviceInfo: req.headers['user-agent'] });
        const createdProduct = await (0, db_1.queryOne)(`SELECT * FROM products WHERE id = ?`, [newId]);
...
exports.productsRouter.put('/:id', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        const { code, name, description, category, unit_measure, price_cop, cost_cop, min_stock, is_active, image_url } = req.body;
        const existingProduct = await (0, db_1.queryOne)(`SELECT * FROM products WHERE id = ?`, [id]);
        if (!existingProduct) {
            res.status(404).json({ success: false, error: 'Producto no encontrado.' });
            return;
        }
        if (!code || !String(code).trim()) {
            res.status(400).json({ success: false, error: 'El código es obligatorio.' });
            return;
        }
        if (!name || !String(name).trim()) {
            res.status(400).json({ success: false, error: 'El nombre del producto es obligatorio.' });
        ...
        const cleanCode = String(code).trim().toUpperCase();
        const codeConflict = await (0, db_1.queryOne)(`SELECT id FROM products WHERE UPPER(code) = ? AND id != ?`, [cleanCode, id]);
        if (codeConflict) {
            res.status(409).json({ success: false, error: `Ya existe otro producto con el código '${cleanCode}'.` });
            return;
        }
        const cleanName = String(name).trim();
        ...
        await (0, db_1.queryRun)(`UPDATE products SET code = ?, name = ?, description = ?, category = ?, unit_measure = ?, price_cop = ?, cost_cop = ?, min_stock = ?, is_active = ?, image_url = ?, updated_at = NOW() WHERE id = ?`, [cleanCode, cleanName, cleanDesc, cleanCategory, cleanUnit, numPrice, numCost, numMinStock, activeStatus, finalImageUrl, id]);
        (0, db_1.recordAuditLog)({ userId: req.user.id, action: 'PRODUCT_UPDATE', entityName: 'products', entityId: String(id), details: { code: cleanCode, name: cleanName, price_cop: numPrice, cost_cop: numCost }, ipAddress: req.ip, deviceInfo: req.headers['user-agent'] });
...
exports.productsRouter.patch('/:id/stock', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        if (!type || !['add', 'subtract', 'set'].includes(type)) {
            res.status(400).json({ success: false, error: "El tipo de ajuste debe ser 'add', 'subtract' o 'set'." });
            return;
        }
        const numQty = Math.round(Number(quantity));
        if (isNaN(numQty) || numQty < 0) {
            res.status(400).json({ success: false, error: 'La cantidad debe ser un número entero mayor o igual a 0.' });
            return;
        }
        ...
        if (type === 'add')
            ...
            if (currentStock < numQty) {
                res.status(400).json({ success: false, error: `No es posible restar ${numQty} unidades. Stock disponible: ${currentStock}.` });
                return;
            }
            newStock = currentStock - numQty;
        }
...
exports.productsRouter.patch('/:id/status', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        ...
        await (0, db_1.queryRun)(`UPDATE products SET is_active = ?, updated_at = NOW() WHERE id = ?`, [newStatus, id]);
        (0, db_1.recordAuditLog)({ userId: req.user.id, action: newStatus === 1 ? 'PRODUCT_ACTIVATE' : 'PRODUCT_DEACTIVATE', entityName: 'products', entityId: String(id), details: { code: existingProduct.code, name: existingProduct.name, is_active: newStatus }, ipAddress: req.ip, deviceInfo: req.headers['user-agent'] });
...
exports.productsRouter.delete('/:id', (0, role_middleware_1.requireRole)(['ADMINISTRADOR']), async (req, res) => {
    try {
        const { id } = req.params;
        const { admin_password } = req.body;
        if (!admin_password) {
            res.status(400).json({ success: false, error: 'Debe ingresar su contraseña de administrador.' });
            return;
        }
        ...
        const product = await (0, db_1.queryOne)('SELECT id, code, name FROM products WHERE id = ?', [id]);
        if (!product) {
            res.status(404).json({ success: false, error: 'Producto no encontrado.' });
            return;
        }
        ...
        (0, db_1.recordAuditLog)({ userId: req.user.id, action: 'PRODUCT_DELETE', entityName: 'products', entityId: String(id), details: { code: product.code, name: product.name, authorizedBy: req.user.username }, ipAddress: req.ip, deviceInfo: req.headers['user-agent'] });
