...
exports.invoicesRouter.get('/stats', async (req, res) => {
    try {
        ...
        const totalStats = await (0, db_1.queryOne)(`SELECT COUNT(*) as total_invoices, COALESCE(SUM(inv.total_cop), 0) as total_revenue_cop, COALESCE(AVG(inv.total_cop), 0) as average_ticket_cop FROM invoices inv ${whereClause}`, params);
        const todayStats = await (0, db_1.queryOne)(`SELECT COUNT(*) as today_invoices, COALESCE(SUM(inv.total_cop), 0) as today_revenue_cop FROM invoices inv ${whereClause} AND date(inv.created_at) = CURRENT_DATE`, params);
        res.json({ success: true, stats: { total_invoices: totalStats?.total_invoices || 0, total_revenue_cop: totalStats?.total_revenue_cop || 0, average_ticket_cop: Math.round(totalStats?.average_ticket_cop || 0), today_invoices: todayStats?.today_invoices || 0, today_revenue_cop: todayStats?.today_revenue_cop || 0, company: exports.MEVACOL_COMPANY_INFO } });
    }
    catch (error) {
...
exports.invoicesRouter.get('/', async (req, res) => {
    ...
    `;
        ...
        const invoices = await (0, db_1.queryAll)(query, params);
...
exports.invoicesRouter.get('/by-sale/:saleId', async (req, res) => {
    try {
        const userRole = req.user.role_code;
        const userId = req.user.id;
        ...
        const invoice = await (0, db_1.queryOne)(`
      SELECT inv.id, inv.invoice_code, inv.sale_id, s.invoice_number as sale_code,
        inv.customer_id, c.name as customer_name, c.id_number as customer_id_number,
        c.phone as customer_phone, c.address as customer_address, c.city as customer_city, c.email as customer_email,
    ...
    `, [invoice.sale_id]);
        res.json({ success: true, invoice: { ...invoice, company: exports.MEVACOL_COMPANY_INFO, items } });
    }
    catch (error) {
...
exports.invoicesRouter.post('/from-sale/:saleId', async (req, res) => {
    try {
        const userRole = req.user.role_code;
        const userId = req.user.id;
        const { saleId } = req.params;
        ...
        const existing = await (0, db_1.queryOne)(`SELECT * FROM invoices WHERE sale_id = ?`, [saleId]);
        if (existing) {
            res.json({ success: true, message: 'La factura ya existía para esta venta.', invoice: existing });
            return;
        }
        const sale = await (0, db_1.queryOne)(`SELECT * FROM sales WHERE id = ?`, [saleId]);
        if (!sale) {
        ...
        const allInvoices = await (0, db_1.queryAll)(`SELECT invoice_code FROM invoices`);
        let maxFac = 0;
        ...
        await (0, db_1.queryRun)(`INSERT INTO invoices (id, invoice_code, sale_id, customer_id, seller_user_id, subtotal_cop, discount_cop, tax_cop, total_cop, notes, dian_status, created_at, updated_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, 0, ?, ?, 'INTERNA', NOW(), NOW())`, [invoiceId, invoiceCode, sale.id, sale.customer_id, sale.seller_user_id, sale.subtotal_cop, sale.discount_cop || 0, sale.total_cop, sale.notes || null]);
        (0, db_1.recordAuditLog)({ userId, action: 'INVOICE_GENERATE', entityName: 'invoices', entityId: invoiceId, details: { invoice_code: invoiceCode, sale_id: sale.id, sale_code: sale.invoice_number, total_cop: sale.total_cop }, ipAddress: req.ip, deviceInfo: req.headers['user-agent'] });
        const newInvoice = await (0, db_1.queryOne)(`SELECT * FROM invoices WHERE id = ?`, [invoiceId]);
