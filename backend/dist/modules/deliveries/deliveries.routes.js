...
exports.deliveriesRouter.use(auth_middleware_1.authMiddleware);
async function generateNextDeliveryCode() {
    const existingDeliveries = await (0, db_1.queryAll)(`SELECT delivery_code FROM deliveries WHERE delivery_code LIKE 'ENT-%'`);
    let maxNum = 0;
...
exports.deliveriesRouter.get('/stats', async (req, res) => {
    try {
...
exports.deliveriesRouter.get('/deliverers', async (req, res) => {
    try {
        const deliverers = await (0, db_1.queryAll)(`SELECT id, full_name, username, phone, email, is_active FROM users WHERE role_code = 'ENTREGADOR' AND is_active = 1 ORDER BY full_name ASC`);
...
exports.deliveriesRouter.get('/available-sales', async (req, res) => {
    try {
        const userRole = req.user.role_code;
        const userId = req.user.id;
        ...
        const availableSales = await (0, db_1.queryAll)(`
...
exports.deliveriesRouter.get('/', async (req, res) => {
    try {
        ...
        const deliveries = await (0, db_1.queryAll)(`
...
exports.deliveriesRouter.get('/:id', async (req, res) => {
    try {
        ...
        const userId = req.user.id;
        const { id } = req.params;
        const delivery = await (0, db_1.queryOne)(`
      SELECT d.id, d.delivery_code, d.sale_id, d.invoice_id, d.customer_id, d.delivery_user_id, d.status,
        d.scheduled_date, d.delivered_at, d.customer_name, d.customer_phone, d.delivery_address, d.delivery_city,
        d.latitude, d.longitude, d.geocoded_at, d.notes, d.created_by_user_id, d.created_at, d.updated_at,
    ...
    `, [id]);
        ...
        const items = await (0, db_1.queryAll)(`SELECT si.id, si.product_id, si.quantity, si.unit_price_cop, si.total_cop, si.notes, p.code as product_code, p.name as product_name, p.unit_measure, p.category FROM sale_items si JOIN products p ON si.product_id = p.id WHERE si.sale_id = ? ORDER BY si.created_at ASC`, [delivery.sale_id]);
        const history = await (0, db_1.queryAll)(`SELECT id, delivery_id, from_status, to_status, changed_by_user_id, changed_by_user_name, notes, created_at FROM delivery_status_history WHERE delivery_id = ? ORDER BY created_at ASC`, [delivery.id]);
...
exports.deliveriesRouter.post('/', async (req, res) => {
    try {
        ...
        const sale = await (0, db_1.queryOne)(`SELECT s.id, s.customer_id, s.seller_user_id, s.status, s.notes, c.name as cust_name, c.phone as cust_phone, c.address as cust_address, c.city as cust_city FROM sales s LEFT JOIN customers c ON s.customer_id = c.id WHERE s.id = ?`, [sale_id]);
        if (!sale) {
        ...
        const existingActiveDelivery = await (0, db_1.queryOne)(`SELECT id, delivery_code, status FROM deliveries WHERE sale_id = ? AND status NOT IN ('CANCELADA')`, [sale_id]);
        if (existingActiveDelivery) {
            res.status(400).json({ success: false, error: `Esta venta ya cuenta con una entrega activa (${existingActiveDelivery.delivery_code}, Estado: ${existingActiveDelivery.status}).` });
            return;
        }
        const invoice = await (0, db_1.queryOne)(`SELECT id, invoice_code FROM invoices WHERE sale_id = ?`, [sale_id]);
        let assignedDeliverer = null;
        let initialStatus = 'PENDIENTE';
        if (delivery_user_id) {
            assignedDeliverer = await (0, db_1.queryOne)(`SELECT id, full_name, role_code, is_active FROM users WHERE id = ?`, [delivery_user_id]);
        ...
        await (0, db_1.withTransaction)(async ({ queryRun: txRun }) => {
            await txRun(`INSERT INTO deliveries (id, delivery_code, sale_id, invoice_id, customer_id, delivery_user_id, status, scheduled_date, customer_name, customer_phone, delivery_address, delivery_city, notes, created_by_user_id, created_at, updated_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [deliveryId, deliveryCode, sale.id, invoice?.id || null, sale.customer_id, assignedDeliverer ? assignedDeliverer.id : null, initialStatus, schedDate, finalCustName, finalCustPhone, finalAddress, finalCity, notes || null, userId]);
            ...
            await txRun(`INSERT INTO delivery_status_history (id, delivery_id, from_status, to_status, changed_by_user_id, changed_by_user_name, notes, created_at) VALUES (?, ?, NULL, ?, ?, ?, ?, NOW())`, [(0, uuid_1.v4)(), deliveryId, initialStatus, userId, userName, historyNotes]);
        });
        (0, db_1.recordAuditLog)({ userId, action: 'CREAR_ENTREGA', entityName: 'deliveries', entityId: deliveryId, details: { delivery_code: deliveryCode, sale_id: sale.id, invoice_code: invoice?.invoice_code, status: initialStatus, delivery_user_id: assignedDeliverer?.id } });
...
exports.deliveriesRouter.patch('/:id/assign', async (req, res) => {
    try {
        const userRole = req.user.role_code;
        const userId = req.user.id;
        const userName = req.user.full_name;
        ...
        const delivery = await (0, db_1.queryOne)(`SELECT * FROM deliveries WHERE id = ?`, [id]);
        if (!delivery) {
            res.status(404).json({ success: false, error: 'Entrega no encontrada.' });
            return;
        }
        if (delivery.status === 'ENTREGADA' || delivery.status === 'CANCELADA') {
            res.status(400).json({ success: false, error: `No se puede reasignar una entrega con estado ${delivery.status}.` });
            return;
        }
        let delivererName = 'Sin asignar';
        let newStatus = delivery.status;
        if (delivery_user_id) {
            const deliverer = await (0, db_1.queryOne)(`SELECT id, full_name, role_code, is_active FROM users WHERE id = ?`, [delivery_user_id]);
        ...
        await (0, db_1.withTransaction)(async ({ queryRun: txRun }) => {
            ...
            await txRun(`INSERT INTO delivery_status_history (id, delivery_id, from_status, to_status, changed_by_user_id, changed_by_user_name, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`, [(0, uuid_1.v4)(), id, delivery.status, newStatus, userId, userName, historyNote]);
        });
        (0, db_1.recordAuditLog)({ userId, action: 'ASIGNAR_ENTREGADOR', entityName: 'deliveries', entityId: String(id), details: { previous_deliverer: delivery.delivery_user_id, new_deliverer: delivery_user_id, new_status: newStatus } });
...
exports.deliveriesRouter.patch('/:id/status', async (req, res) => {
    try {
        ...
        if ((status === 'NO_ENTREGADA' || status === 'CANCELADA') && (!notes || !notes.trim())) {
            res.status(400).json({ success: false, error: `Para marcar como ${status} es obligatorio indicar la observación o motivo.` });
            return;
        }
        ...
        await (0, db_1.withTransaction)(async ({ queryRun: txRun }) => {
            ...
            await txRun(`INSERT INTO delivery_status_history (id, delivery_id, from_status, to_status, changed_by_user_id, changed_by_user_name, notes, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())`, [(0, uuid_1.v4)(), id, delivery.status, status, userId, userName, historyNote]);
        });
        (0, db_1.recordAuditLog)({ userId, action: 'CAMBIO_ESTADO_ENTREGA', entityName: 'deliveries', entityId: String(id), details: { from_status: delivery.status, to_status: status, notes: historyNote } });
